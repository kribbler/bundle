	...
	(function($) {
		...
		$.widget("pexeto.pexetoColorpicker", {
			...
			init: function() {
				...
				this.element.on("keyup", this._doOnKeyUp);
			},
			...
			/**
			 * On key up event handler - updates the preview color value when the input value changes.
			 */
			_doOnKeyUp: function() {
		...
		$.widget("pexeto.pexetoUpload", {
			 ...
			 */
			init: function() {
				new AjaxUpload(this.element.get(0), {
					action: pexeto.addUrlParam(ajaxurl, "action", "pexeto_upload"),
					name: 'pexetofile',
					onSubmit: this._doOnSubmit,
					onComplete: this._doOnUploadComplete
				});
			},
			 ...
			 */
			_doOnUploadComplete: function(file, response) {
				var res = $.parseJSON(response),
					errorText = "";
				if(res.success && res.uploadurl && res.filename) {
					//the upload was successful
					fileUrl = res.uploadurl + '/' + res.filename;
			...
			/**
			 * On upload submit callback - adds the loading class to show
			 * that the upload is in process.
			 */
			_doOnSubmit: function() {
