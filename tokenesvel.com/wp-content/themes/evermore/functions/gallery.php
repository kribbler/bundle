 ...
 * OTHER FUNCTIONS:
 *
 *  pexeto_get_gallery_thumbnail_width
 */
...
if ( !function_exists( 'pexeto_get_gallery_thumbnail_html' ) ) {
	...
	function pexeto_get_gallery_thumbnail_html( $post, $columns, $image_height, $itemclass='pg-item' ) {
		...
		$image_width = pexeto_get_gallery_thumbnail_width( $columns, 'full' );
		$settings = pexeto_get_post_meta( $post->ID, array( 'type', 'crop' ), PEXETO_PORTFOLIO_POST_TYPE );
		$exclude_info = pexeto_option( 'portfolio_exclude_info' );
		$add_class = sizeof( $exclude_info ) == 1 ? ' pg-info-dis' : '';
	...
	function pexeto_get_portfolio_slider_item_html( $itemid, $single=true ) {
		...
		if ( !empty( $post ) ) {
			...
			if ( !$single ) {
				...
				$prev_text = str_replace( ' ', '<br/>', pexeto_text( 'prev_project_text' ) );
				$html.='<a href="#" rel="prev" class="ps-prev-project-link'.$p_dis_class
					.'"><span class="ps-icon"></span><span>'.$prev_text.'</span></a>';
				$html.='<a href="#" rel="back" class="ps-back-link" title="'
					.pexeto_text( 'back_to_gallery_text' ).'"><span class="ps-icon"></span></a>';
				if ( ( isset( $_GET['next'] ) && ( $_GET['next']==='false' || $_GET['next']==false ) ) ) {
					$n_dis_class=' disabled';
				}
				$next_text = str_replace( ' ', '<br/>', pexeto_text( 'next_project_text' ) );
				$html.='<a href="#" rel="next" class="ps-next-project-link'.$n_dis_class.'"><span class="ps-icon"></span><span>'.$next_text.'</span></a>';
...
if ( !function_exists( 'pexeto_get_portfolio_carousel_html' ) ) {
	...
	function pexeto_get_portfolio_carousel_html( $itemid ) {
		...
		$html = pexeto_build_portfolio_carousel_html( $posts, pexeto_text( 'portfolio_carousel_title' ) );
		return $html;
	}
}
...
if ( !function_exists( 'pexeto_get_gallery_thumbnail_width' ) ) {

	/**
	 * Retrieves the image thumbnail width according to the number of columns
	 * and the layout of the current page.
	 * @param  int  $columns the number of columns the images will be located into
	 * @param  string  $layout  the layout of the page
	 * @param  int $margin  the space between the images
	 * @return int           the calculated image width
	 */
	function pexeto_get_gallery_thumbnail_width( $columns, $layout, $margin=15 ) {
		global $pexeto_content_sizes;
		...
		switch ( $layout ) {
		case 'full':
			$layout_key = pexeto_contains_posts() ? 'blog_fullwidth' : 'fullwidth';
			$page_width = $pexeto_content_sizes[$layout_key];
			break;
		case 'twocolumn':
		case 'threecolumn':
			$page_width = $pexeto_content_sizes[$layout]-$pexeto_content_sizes['column_spacing'];
			break;
		default:
			$layout_key = pexeto_contains_posts() ? 'blog_content' : 'content';
			$page_width = $pexeto_content_sizes[$layout_key];
			break;
		}

		//calculate the image width according to the page width and the number of columns
		$image_width = ( $page_width+$margin ) / ( $columns ) - $margin - $additional_space;
...
if ( !function_exists( 'pexeto_print_gallery' ) ) {
	...
	function pexeto_print_gallery( $output, $attr ) {
		...
		$image_width = pexeto_get_gallery_thumbnail_width( $columns, $layout, 10 );
		...
		foreach ( $attachments as $attachment ) {
			$html .= '<div class="qg-img" data-defwidth="'.$image_width.'"><a href="'
			.$attachment->guid.'" rel="lightbox[group-'.$post->ID.']" title="'
			.htmlspecialchars( $attachment->post_content ).'" ><img src="'
			.pexeto_get_resized_image( $attachment->guid, $image_width, $image_height ).'" />';
			$html.='<span class="icon-circle"><span class="pg-icon lightbox-icon"></span></span>';
