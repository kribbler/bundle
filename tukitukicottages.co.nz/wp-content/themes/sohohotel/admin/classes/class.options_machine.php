...
class Options_Machine {
			...
			switch ( $value['type'] ) {
				...
				case 'select':
					...
					foreach ($value['options'] as $select_ID => $option) {			
						$output .= '<option id="' . $select_ID . '" value="'.$option.'" ' . selected(!empty($smof_data[$value['id']]) ? $smof_data[$value['id']] : '', $option, false) . ' />'.$option.'</option>';
					 } 
					$output .= '</select></div>';
				break;
				...
				case 'custom_blocks':
					...
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						$output .= Options_Machine::optionsframework_custom_blocks_function($value['id'],$value['std'],$oldorder,$order,$int);
					} else {
						$i = 0;
						foreach ($slides as $slide) {
							$oldorder = $slide['order'];
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button block_add_button">Add New Block</a></div>';
	 ...
	 * @return string
	 */
	public static function optionsframework_custom_blocks_function($id,$std,$oldorder,$order,$int){
		...
		//initialize all vars
		$slidevars = array('title','url','link','colour');
		...
		if (!empty($val['title'])) {
			...
			$slider .= '<li><div class="slide_header"><strong>Block '.$order.'</strong>';
		}
		...
		$slider .= '<label>Background Colour (e.g. #000000)</label>';
		$slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][colour]" id="'. $id .'_'.$order .'_slide_colour" value="'. $val['colour'] .'" />';
