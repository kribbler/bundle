  ...
  function ot_type_demo_data( $args = array() ) {
    ...
    echo '<div class="format-setting type-backup ' . ( $has_desc ? 'has-desc' : 'no-desc' ) . '">';
        ...
        if($demo_data_installed == 'yes') { 
            $button_label = __('Update Options', ETHEME_DOMAIN);
        } else {
            $button_label = __('Install Demo Data', ETHEME_DOMAIN);
        }
    	...
    	?>
            <select class="option-tree-ui-select" id="demo_data_style">
                <?php
                    foreach ( (array) $field_choices as $choice ) {
                      if ( isset( $choice['value'] ) && isset( $choice['label'] ) ) {
                        echo '<option value="' . esc_attr( $choice['value'] ) . '">' . esc_attr( $choice['label'] ) . '</option>';
                      }
                    }
                ?>
            </select>
        ...
        if($demo_data_installed == 'yes') {
            ?>
                <div class="clear"></div>
                <br />
                <p><?php _e('<strong>Note:</strong> You have already installed demo content. Now you can just update your theme options (colors, header, footer, typography)', ETHEME_DOMAIN) ?></p>
            <?php
        }
