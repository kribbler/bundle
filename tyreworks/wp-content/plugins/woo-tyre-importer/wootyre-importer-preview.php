...
$file_number = 1;
	...
	function set_header_row(){
        ...
        $header_row[] = 'post_type';
        $header_row[] = 'product_type';
		$header_row[] = 'name';
		$header_row[] = 'pa_brand';
		$header_row[] = '';////////////////////
		$header_row[] = 'sku';
        $header_row[] = 'pa_service';
    ...
    if($file_path) {
		...
		if ( $handle !== FALSE ) {
            while ( ( $line = fgetcsv($handle) ) !== FALSE ) {
            	$k++;
				if ($file_start){
					$myfile = fopen($upload_dir['basedir'] . '/csv_import/' . $new_file_name . '___' . $file_number . '.csv', "w") or die('error');
					fwrite($myfile, implode(",", set_header_row()) . "\n");
					fclose($myfile);
					if ($k != 1){
						$myfile = fopen($upload_dir['basedir'] . '/csv_import/' . $new_file_name . '___' . $file_number . '.csv', "a") or die('error');
						$file_start = false;
...
//var_dump($import_data[0]); die();
    ...
    $col_mapping_options = array(
        ...
        'post_type' => array(
            'label' => __('Post Type', 'wootyre-importer'),
            'mapping_hints' => array('post_type', 'post type')),
        'post_status' => array(
        ...
        '_product_type' => array(
            ...
            'mapping_hints' => array('product type', 'type', '_product_type')),
