...
//echo "<pre>";var_dump($_FILES); var_dump($_POST);die();
    ...
    $post_data = array(
    	...
    	'post_status' => maybe_unserialize(stripslashes($_POST['post_status']))
    );
    ...
    if(isset($post_data['uploaded_file_path'])) {
        ...
        if ( $handle !== FALSE ) {
            ...
            $error_messages[] = __( 'Could not open CSV file.', 'wootyre-importer' );
        }

        if(sizeof($import_data) == 0) {
            $error_messages[] = __( 'No data found in CSV file.', 'wootyre-importer' );
        }
        ...
        foreach($import_data as $row_id => $row) {
            ...
            $new_post_defaults['post_status'] = $post_data['post_status'];
            ...
            foreach($row as $key => $col) {
                ...
                switch($map_to) {
					...
					case 'custom_field':
                        ...
                        $fs = explode('|', $col);
                        foreach ($fs as $v){
                        	$new_post_custom_fields[$v] = array (
	                            "name" => $field_name,
	                            "value" => $v,
                                ...
                                //var_dump($fs);var_dump($post_data['custom_field_name'][$key]);
	                        $new_post_terms[$post_data['custom_field_name'][$key]] = $fs;
                                if ($post_data['custom_field_name'][$key] == 'pa_seller-sku'){
                                    $current_seller_sku = $fs[0];
                                }
                        }
                        break;
...
//echo "<pre>";var_dump($new_post_custom_fields);die();
            /**
             * DANIEL:: check for matching seller SKU. If found, then the old product must be deleted!!!
             */
            ...
            $args = array(
                ...
                'tax_query' => array(
                    array(
                        'taxonomy' => 'pa_seller-sku',
                        'terms' => explode(",",$current_seller_sku),
                        'field' => 'slug',
                        'operator' => 'IN'
                    )
                )
            ...
            **/
            ...
            if(strlen($new_post['post_title']) > 0 || $existing_product !== null) {
                ...
                if($existing_product !== null) {
                    $new_post_messages[] = sprintf( __( 'Updating product with ID %s.', 'wootyre-importer' ), $existing_product->ID );
                ...
                if(is_wp_error($new_post_id)) {
                    $new_post_errors[] = sprintf( __( 'Couldn\'t insert product with name %s.', 'wootyre-importer' ), $new_post['post_title'] );
                } elseif($new_post_id == 0) {
                    $new_post_errors[] = sprintf( __( 'Couldn\'t update product with ID %s.', 'wootyre-importer' ), $new_post['ID'] );
                } else {
                    //insert successful!
                    ...
                    if($existing_product !== null) {
                        ...
                        //var_dump($existing_product_attributes);die();
                    ...
                    foreach($new_post_image_urls as $image_index => $image_url) {
                        ...
                        if(!in_array($image_ext, $allowed_extensions)) {
                            $new_post_errors[] = sprintf( __( 'A valid file extension wasn\'t found in %s. Extension found was %s. Allowed extensions are: %s.', 'wootyre-importer' ), $image_url, $image_ext, implode( ',', $allowed_extensions ) );
                            continue;
                        }
                        ...
                        if(ini_get('allow_url_fopen')) {
                            ...
                            if( ! @copy($image_url, $dest_path)) {
                                ...
                                $new_post_errors[] = sprintf( __( '%s encountered while attempting to download %s', 'wootyre-importer' ), '{$http_status}', $image_url );
                            }
                            ...
                            if($http_status != 200) {
                                unlink($dest_path);
                                $new_post_errors[] = sprintf( __( 'HTTP status %s encountered while attempting to download %s', 'wootyre-importer' ), '{$http_status}', $image_url );
                            }
                        } else {
                            //well, damn. no joy, as they say.
                            $error_messages[] = sprintf( __( 'Looks like %s is off and %s is not enabled. No images were imported.', 'wootyre-importer' ), '<code>allow_url_fopen</code>', '<code>cURL</code>'  );
                            break;
                        }

                        //make sure we actually got the file.
                        if(!file_exists($dest_path)) {
                            $new_post_errors[] = sprintf( __( 'Couldn\'t download file %s.', 'wootyre-importer' ), $image_url );
                            continue;
                        }
                    ...
                    foreach($new_post_image_paths as $image_index => $dest_path_info) {
                        ...
                        if($existing_product !== null && intval($post_data['product_image_skip_duplicates'][$key]) == 1) {
                            ...
                            if(is_array($existing_attachments) && sizeof($existing_attachments) > 0) {
                                ...
                                $new_post_messages[] = sprintf( __( 'Skipping import of duplicate image %s.', 'wootyre-importer' ), $dest_path_info['source'] );
                                continue;
                            }
                        }
                        ...
                        if(!file_exists($dest_path_info['path'])) {
                            $new_post_errors[] = sprintf( __( 'Couldn\'t find local file %s.', 'wootyre-importer' ), $dest_path_info['path'] );
                            continue;
                        }
                ...
                $new_post_errors[] = __( 'Skipped import of product without a name', 'wootyre-importer' );
            }
